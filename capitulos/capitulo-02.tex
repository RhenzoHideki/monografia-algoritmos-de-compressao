...
prefixo, sendo a maior sequência de caracteres iguais, que ocorra dentro do \textit{lookahead buffer}. O resultado dessa
...
definido em 6 caracteres e \textit{search buffer} de 7 caracteres. O estado inicial da janela pode ser visto na
...
\begin{figure}[ht]
  ...
  \includegraphics[width=15cm]{figuras/DiagramasTCC-LZ77-Estado-1 (1).pdf}
  \fonte{Adaptada de \textcite{sayood2012introduction}}
\end{figure}
...
\begin{figure}[htp]
  \centering
  \caption{Estado da janela após primeira codificação no algoritmo LZ77}
  \label{fig:Estado_2_LZ77}
  \includegraphics[width=15cm]{figuras/DiagramasTCC-LZ77-Estado-2 (1).pdf}
  \fonte{Adaptada de \textcite{sayood2012introduction}}
\end{figure}

Após a segunda codificação, a janela deslizante avança novamente, e o
processo se repete. Procura-se no \textit{search buffer} a maior correspondência para o início do
\textit{lookahead buffer}, que agora começa com \texttt{"rar"}. Neste caso, a
o deslocamento até a correspondência mais longa é 4, com comprimento 3
(\texttt{"rar"}), seguido do caractere literal \texttt{"r"}. Assim, o token
gerado é $\langle 4, 3, \texttt{r} \rangle$, este processo pode ser observado na
Figura~\ref{fig:Estado_2_LZ77}.
...
Inicialmente, ele utiliza o pointer ($p$) para reconstruir, neste caso na posição 0 do
\textit{search buffer}
, a sequência já decodificada até o momento. A partir dessa posição inicial
...
\begin{itemize}
  ...
  \item \textbf{Tunstall Code}: esquema de comprimento fixo sobre alfabetos
        de saída maiores, utilizado em compressão de texto;
...
disponíveis, ampliando a família de algoritmos \textit{Lempel–Ziv}.
